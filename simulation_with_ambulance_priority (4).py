...
defaultRed = 150
defaultYellow = 5
defaultGreen = 40  # increased from 20 to 40 for more green light duration
defaultMinimum = 10
defaultMaximum = 60
...
speeds = {'ambulance': 3, 'bus': 1.8, 'truck': 1.8, 'rickshaw': 2, 'bike': 2.5}  # average speeds of vehicles
...
vehicleTypes = {0: 'ambulance', 1: 'bus', 2: 'truck', 3: 'rickshaw', 4: 'bike'}
...
def setTime():
    ...
    greenTime = math.ceil(((noOfAmbulances * 2 * carTime) + (noOfCars * carTime) + (noOfRickshaws * rickshawTime) + (noOfBuses * busTime) + (noOfTrucks * truckTime) + (noOfBikes * bikeTime)) / (noOfLanes + 1))
    ...
    greenTime = math.ceil(((noOfAmbulances * 2 * carTime) + (noOfCars * carTime) +
...
def get_ambulance_priority_direction():
    max_count = 0
    priority_dir = -1
    for idx, direction in directionNumbers.items():
        ...
        for lane in range(3):
            for vehicle in vehicles[direction][lane]:
                if vehicle.vehicleClass == 'ambulance' and not vehicle.crossed:
                    count += 1
        if count > max_count:
            max_count = count
            priority_dir = idx
...
def repeat():
    global currentGreen, currentYellow, nextGreen
        ...
        # 🚨 Ambulance priority logic
    priority_dir = get_ambulance_priority_direction()
    if priority_dir != -1 and priority_dir != currentGreen:
        print(f"\n🚨 Priority given to ambulances at {directionNumbers[priority_dir]} lane\n")
        currentGreen = priority_dir
        nextGreen = (currentGreen + 1) % noOfSignals
